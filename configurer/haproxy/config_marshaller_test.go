...
frontend frontend_cfg_80
  mode tcp
  bind :80
  default_backend backend_cfg_80

backend backend_cfg_80
  mode tcp
...
frontend frontend_cfg_80
  ...
  use_backend backend_cfg_80_external-host.example.com if { req.ssl_sni external-host.example.com }

backend backend_cfg_80_external-host.example.com
  mode tcp
  server default-host.internal:8080
`))
			})
...
`))
		...
		Context("when there are multiple inbound ports", func() {
			It("sorts the inbound ports", func() {
				...
				Expect(marshaller.Marshal(haproxyConf)).To(Equal(`
frontend frontend_cfg_70
  mode tcp
  bind :70
  default_backend backend_cfg_70

backend backend_cfg_70
  mode tcp
...
frontend frontend_cfg_90
  mode tcp
  bind :90
  default_backend backend_cfg_90

backend backend_cfg_90
  mode tcp
...
frontend frontend_cfg_80
  ...
  tcp-request content accept if { req.ssl_hello_type gt 0 }
  default_backend backend_cfg_80
  use_backend backend_cfg_80_host-1.example.com if { req.ssl_sni host-1.example.com }
  use_backend backend_cfg_80_host-99.example.com if { req.ssl_sni host-99.example.com }

backend backend_cfg_80
  mode tcp
  server default-host.internal:8080

backend backend_cfg_80_host-1.example.com
  mode tcp
  server host-1.internal:1111

backend backend_cfg_80_host-99.example.com
  mode tcp
